...
namespace Dickson.Web
{
    public class WebBrowser
    {
        ...
        Lazy<bool> m_IsWeChat;
        Lazy<bool> m_IsMobileDevice;

        public WebBrowser(string agent)
        {
            var nullAgent = string.IsNullOrWhiteSpace(agent);
            m_IsWeChat = new Lazy<bool>(() =>
            {
                return nullAgent ? false : (agent.Contains(" MicroMessenger/"));
            });
            m_IsMobileDevice = new Lazy<bool>(() =>
            {              
                return nullAgent ? false : ((b.IsMatch(agent) || v.IsMatch(agent.Substring(0, 4)))); ;
            });
        }
