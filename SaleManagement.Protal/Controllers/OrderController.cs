    ...
    public class OrderController : PortalController
    {
        ...
        public async Task<JsonResult> Booking(OrderEditViewModel request, [NamedModelBinder(typeof(CommaSeparatedModelBinder), "attachmentIds")] string[] attachmentIds)
        {
            ...
            if (!string.IsNullOrEmpty(request.VersionNo))
            {
                ...
                var hotSelling = await hotSellingManager.GetHotSellingByNoAsync(request.VersionNo);
                order.Attachments = new List<OrderAttachment>();
                hotSelling.Attachments.ForEach(a =>
                    order.Attachments.Add(new OrderAttachment
                    {
                        FileInfoId = a.FileInfoId,
                        OrderId = order.Id,
                        CreatorId = User.Id
                    }));
                order.ModuleType = ModuleType.Exist; //直接出腊
                order.OrderStatus = OrderStatus.CustomerConfirm;
            }
            var result = await manager.CreateOrder(order);
            if (result.Succeeded)
            {
                var operationLogManager = new OrderOperationLogManager(User);
                var orderStatus = string.IsNullOrEmpty(request.VersionNo) ? OrderStatus.UnConfirmed
                    : OrderStatus.CustomerConfirm;

                await operationLogManager.AddLogAsync(orderStatus, order.Id);
